...
//        }
        ...
        // 단, 중복숫자는 허용하지 않음
        System.out.println("-------로또 작성-------");
        Set<Integer> lotto = new HashSet<>(); // lotto라는 해시셋 생성

        int i = 0;
        while (i != 6) { // 6번을 반복
            i++;
            while (lotto.size() < 6) { // lotto 사이즈가 6번이 안되면 아래 코드 계속 실행
                lotto.add((int) (Math.random()*45+1)); // 중복이면 자동으로 빠지니 lotto 사이즈가 6번이 되지 않을거고 계속 진행 시킴
            }
        }

        System.out.println("로또 번호 개수는 "+lotto.size()+"개"); // 확인 프린트
