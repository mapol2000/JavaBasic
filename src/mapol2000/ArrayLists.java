...
public class ArrayLists {
    ...
    public static void main(String[] args) {
        ...
        // 전체를 흩어가며 대상을 찾기때문에 성능 나쁨
        for (int i = 0; i < games.size(); ++i) { // 동적표현에서는 size를 씀
        ...
        // 따라서 실제 GameInfo 객체의 실제값을 출력하려면
        ...
        for (GameInfo g : games2) {
            if (g.name.equals("스타크래프트")) {
                System.out.println("foreach문으로 데이터 찾음");
            }
        }

            for (GameInfo g : games2) {
                if (g.name.contains("스타크래프트")) {
                    System.out.println("contains문으로 데이터 찾음");
                }
            }


        for (GameInfo g : games2) {
            if (g.name.indexOf("스타크래프트") >= 0) {
                System.out.println("indexOf문으로 데이터 찾음");
            }
        }
